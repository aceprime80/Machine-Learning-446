...
class ConvNet(nn.Module):
    ...
    def __init__(self, kernel_size=3, length=10, out_chan=32, bias=True):
        ...
        self.length = length
        self.conv = nn.Conv1d(1,out_chan,bias=bias,kernel_size=kernel_size)
        self.convT = nn.ConvTranspose1d(out_chan, 1, bias=bias,kernel_size=kernel_size)

    def forward(self, x):
        x = x.reshape((-1,1,self.length))
        x = (self.convT(F.relu(self.conv(x))))
        #print(x.shape)
...
class Attention(nn.Module):
    ...
    def compute_new_values(self, q, k, v):
        ...
        #print(k.shape)
        #print(q.shape)
        attentions = F.softmax((k@q.transpose(1,2))/np.sqrt(self.embedding_dim),2)
        values = attentions @ v
...
def test(model, testloader, log=True):
    ...
    for batch_idx, inputs in enumerate(testloader):
        ...
        predictions = torch.clip(outputs,0,1)
        predictions = (predictions>0.5).float()
        #print(torch.prod(predictions.eq(Y).float(), dim=1))
...
if __name__ == "__main__":
    ...
    for epoch in range(25):
        acc = train(model, epoch, optimizer, criterion, trainloader)
        k3acc.append(acc)
        #if epoch % 10 == 0 and epoch > 0:
        #    test(model, testloader)
    att_mat = model.attention_mat()
    print(att_mat)
    print(att_mat.shape)
    print(att_mat.dtype)
    plt.imshow(att_mat)
    ...
    for epoch in range(25):
        ...
        if epoch % 10 == 0 and epoch > 0:
            test(model, testloader)
    plt.title("Attention Network Training Accuracies")
    
    plt.plot(k3acc, label = "Positional Encoding = True")
    plt.plot(k10acc, label = "Postional Encoding = False")
